...
class Summary
{
    ...
    protected function outputArguments($arguments, $type)
    {
        ...
        $this->console->br()->out(mb_convert_case($type, MB_CASE_TITLE) . ' Arguments:');

        foreach ($arguments as $argument) {
            $this->console->tab()->out($this->argument($argument));

            if ($argument->description()) {
                $this->console->tab(2)->out($argument->description());
            }
        }
    }
