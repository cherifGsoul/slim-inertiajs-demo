...
class Progress extends DynamicTerminalObject
{
     ...
     */
    public function current(int $current, string $label = ''): void
    {
        if ($this->total == 0) {
            // Avoid dividing by 0
     ...
     * @param string $label
     */
    public function advance(int $increment = 1, string $label = ''): void
    {
     ...
     * @return Progress
     */
    public function forceRedraw(bool $force = true): Progress
    {
     ...
     */
    public function each(iterable $items, callable $callback = null)
    {
    ...
    {
        $percentage = intval($current / $this->total);
        $percentage = $this->percentageFormatted($percentage);
     ...
     * @return string
     */
    protected function getProgressBar(int $current, string $label)
    {
        if ($this->first_line) {
     ...
     * @return string
     */
    protected function percentageFormatted(int $percentage): string
    {
